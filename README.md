...
This repository contains the complete development lineage, from foundational baseline bots to top-tier competitive tournament contenders: **Chimera**, **Magnus**, and **NewNewNeuro**.
...
## Contender Agents and Architecture
...
| **`Magnus`** | Monte Carlo Tree Search (MCTS) + Bayesian Trapdoor Filtering + Killer Heuristics | Deep tactical tree rollouts; multi-step horizon lookahead and trap risk minimization. |
...
| **`Cordelia`** | Anti-diagonal Barrier Placement + Heuristic Pruning | Defensive zoning specialist that isolates opponent territory using strategic barrier mechanics. |
...
- Automatically isolates confirmed trapdoors while dynamically weighting soft risk thresholds (e.g., pruning paths with $P(\text{trap}) \ge 0.18$).
...
- **Endgame Phase (≥ 29 turns)**: Defense, score protection, and opponent entrapment.
