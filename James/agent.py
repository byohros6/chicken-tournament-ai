...
class PlayerAgent:
    ...
    def play(
        ...
        # 1. OPENING: SPRINT TO CORNER (with egg laying along the way!)
        # CRITICAL FIX: Always lay eggs when on an egg square, even while rushing!
        my_loc = board.chicken_player.get_location()
        corners = [(0,0), (0,7), (7,0), (7,7)]
        my_parity = board.chicken_player.even_chicken
        valid_corners = [c for c in corners if (c[0]+c[1])%2 == my_parity]

        # ALWAYS lay an egg if we can - this is the #1 priority!
        if board.can_lay_egg():
            egg_moves = [m for m in board.get_valid_moves() if m[1] == enums.MoveType.EGG]
            if egg_moves:
                # Pick the egg move that gets us closest to a corner (if rushing) or just lay
                free_corners = [c for c in valid_corners if c not in board.eggs_player]
                if free_corners:
                    target = min(free_corners, key=lambda c: abs(c[0]-my_loc[0]) + abs(c[1]-my_loc[1]))
                    # Sort egg moves by distance to target after the move
                    best_egg = min(egg_moves, key=lambda m: abs(target[0] - self._get_position_after_move(my_loc, m[0])[0]) + abs(target[1] - self._get_position_after_move(my_loc, m[0])[1]))
                    return best_egg
                else:
                    return egg_moves[0]  # Just lay the egg

        # Are any of our corners still empty?
        free_corners = [c for c in valid_corners if c not in board.eggs_player]

        if free_corners and board.turn_count < 30:
            # Target the closest free corner
            target = min(free_corners, key=lambda c: abs(c[0]-my_loc[0]) + abs(c[1]-my_loc[1]))

            # Pathfind to corner - prefer moves that get us closer
            best_rush = None
            min_dist = 999
            
            for move in board.get_valid_moves():
                d, t = move
                next_pos = self._get_position_after_move(my_loc, d)
                
                # Safety: Only avoid KNOWN trapdoors, not suspected ones (too cautious)
                if next_pos in board.found_trapdoors: continue
                if next_pos in board.eggs_player: continue  # Don't step on own eggs
                
                dist_after = abs(target[0]-next_pos[0]) + abs(target[1]-next_pos[1])
                
                # Only consider moves that get us closer (or equal for turds on enemy tiles)
                if dist_after < min_dist:
                    min_dist = dist_after
                    best_rush = move
                elif dist_after == min_dist and best_rush is not None:
                    # Same distance - prefer PLAIN over TURD (save turds for later)
                    if t == enums.MoveType.PLAIN and best_rush[1] == enums.MoveType.TURD:
                        best_rush = move
                    
            if best_rush: return best_rush
        ...
        # CRITICAL: Filter out moves that would step on KNOWN trapdoors only
        # Don't avoid suspected trapdoors - that makes us too cautious
        safe_moves = []
        ...
        # TURD STRATEGY UPDATE: Allow turds if adjacent OR if on enemy tile (Drive-By)
        enemy_pos = board.chicken_enemy.get_location()
        dist_to_enemy = abs(current_pos[0] - enemy_pos[0]) + abs(current_pos[1] - enemy_pos[1])
        enemy_parity = board.chicken_enemy.even_chicken
        current_parity = (current_pos[0] + current_pos[1]) % 2

        # Filter turd moves
        turd_moves = [m for m in valid_moves if m[1] == enums.MoveType.TURD]

        # If opponent is NOT adjacent, we usually remove turds...
        if dist_to_enemy != 1:
            # ...BUT keep them if we are standing on an enemy tile (Free Action!)
            if current_parity == enemy_parity:
                pass # Keep turd moves allowed!
            else:
                # We are on our own tile or safe tile, don't waste turn laying turd if enemy isn't close
    ...
    def evaluate(self, board_state: board.Board) -> float:
        ...
        turd_bonus = 0
        enemy_parity = board_state.chicken_enemy.even_chicken

        for turd_pos in board_state.turds_player:
            # 1. Did we drop it on an Enemy Tile? (Drive-By)
            # If the turd is on a tile the enemy WANTS (their parity), that is huge value.
            if (turd_pos[0] + turd_pos[1]) % 2 == enemy_parity:
                # Check if it's "useful" (not on the edge of the map)
                if 1 < turd_pos[0] < 6 and 1 < turd_pos[1] < 6:
                    turd_bonus += 150  # Great placement! Blocked a central enemy tile.
                else:
                    turd_bonus += 50   # Okay placement.
                    
            # 2. Is it blocking the enemy RIGHT NOW?
            dist_to_enemy = abs(turd_pos[0] - enemy_pos[0]) + abs(turd_pos[1] - enemy_pos[1])
            if dist_to_enemy <= 2:
                turd_bonus += 300 # Active Combat Block
